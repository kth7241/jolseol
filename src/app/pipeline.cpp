...
// 상대경로 실행 위치 차이(build/ 또는 프로젝트 루트) 때문에
// 파일을 못 찾는 경우가 많아서, 몇 가지 후보 경로를 순서대로 확인한다.
std::string resolveExistingPath(const std::string& raw_path) {
...
int Pipeline::run() {
  ...
  // 2) Detector
  // 우선순위:
  //   1. yolo_onnx_path가 있으면 실제 YOLO ONNX detector 사용
  //   2. 없거나 생성 실패 시 기존 stub detector로 fallback
  const std::string resolved_yolo_path = resolveExistingPath(opt_.yolo_onnx_path);
  if (!opt_.yolo_onnx_path.empty()) {
    try {
      traffic::detect::OnnxYoloDetector::Options yolo_opt;
      yolo_opt.model_path = resolved_yolo_path;
      yolo_opt.input_width = opt_.yolo_input_w;
      yolo_opt.input_height = opt_.yolo_input_h;
      yolo_opt.conf_threshold = opt_.yolo_conf_thresh;
      yolo_opt.nms_threshold = opt_.yolo_nms_thresh;
      yolo_opt.try_cuda = opt_.yolo_try_cuda;
      yolo_opt.debug_shape = opt_.yolo_debug_shape;

      impl_->detector = std::make_unique<traffic::detect::OnnxYoloDetector>(yolo_opt);
      TM_LOGI("[Pipeline] YOLO ONNX detector enabled: %s", resolved_yolo_path.c_str());
    } catch (const std::exception& e) {
      TM_LOGW("[Pipeline] YOLO detector init failed -> fallback to stub: %s", e.what());
      impl_->detector = traffic::detect::CreateDetectorStub();
    }
  } else {
    TM_LOGW("[Pipeline] yolo_onnx_path is empty -> using detector stub");
    impl_->detector = traffic::detect::CreateDetectorStub();
  }
  ...
  // 4) IPM config/warper
  const std::string resolved_ipm_path = resolveExistingPath(opt_.ipm_yaml_path);
  if (!opt_.ipm_yaml_path.empty()) {
    if (!traffic::load_ipm_config(resolved_ipm_path, impl_->ipm_cfg)) {
  ...
  const std::string resolved_zones_path = resolveExistingPath(opt_.zones_json_path);
  if (!opt_.zones_json_path.empty()) {
    if (!impl_->zone_mgr.load(resolved_zones_path)) {
      LOGW("[Pipeline] zone load failed. Zone gating may not work.");
    }
  } else {
    LOGW("[Pipeline] zones_json_path empty. Zone gating disabled.");
  }
